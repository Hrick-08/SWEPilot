...
//   const [run, setRun] = useState<Awaited<ReturnType<typeof runsService.getById>>>();
//   const issueId = run?.issueId ?? 0;
//   const { logs, isStreaming } = useIssueLogs(issueId);
//   const files = changedFiles[issueId] ?? [];
//   const tests = testResults[issueId] ?? [];

//   useEffect(() => {
//     if (!runId) return;
//     void runsService.getById(runId).then(setRun).catch(() => setRun(undefined));
//   }, [runId]);
